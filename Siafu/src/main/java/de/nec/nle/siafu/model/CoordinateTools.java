...
public class CoordinateTools {
	...
	public String localToPrettyCoordinates(final Position pos) {
		String latitude = String.valueOf(pos.getCoordinates()[0]).replace(",", ".");
		String longitude = String.valueOf(pos.getCoordinates()[1]).replace(",", ".");
		System.out.println(String.format("Coordenada: %s, %s",latitude, longitude));

		String coordenada = latitude+", "+longitude;
		/*
			Publicar no tópico do Broker a coordenada
			Tópico:  pet/id_pet/sensor/nome_sensor/coordenada
			Exemplo: pet/001/sensor/gps/-5.2323, -41.3852
		 */
